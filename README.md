...
> 📣 **欢迎加入 QQ 群帮忙测试！群号：`1058830118`** 🎮
>
> 项目仍在开发阶段，需要你的反馈和建议来改进。点击加入：[QQ 群 1058830118](https://qm.qq.com/q/1058830118)
...
- **多语言格式**：支持 `.json` 和 `.properties` / `.lang` 两种语言文件格式，自动识别并对应打包
- **智能提取**：
  - 自动扫描模组语言文件，智能选择源语言（变体优先 > 英语优先 > 字典序）
  - 跳过 Minecraft 本体，仅提取额外模组
  - 目标语言原生模组自动跳过
  - 支持差异提取 / 全量提取
  - 提取时检查词典，已命中的不加入 AI 文件，减少翻译量
- **多种翻译方式**：
  - 词典翻译（I18n-dict）：外置目录加载，支持多文件合并（仅限目标语言为中文时生效）
  - AI 手动翻译：生成 AI 待翻译文件，人工粘贴翻译结果
  - AI 自动翻译：通过 API 自动调用 AI 翻译
  - 机器翻译（MT）：通过 API 调用机器翻译服务
- **高效翻译**：仅处理未翻译的键，避免重复翻译
- **优先级管理**：用户翻译 > 词典翻译 > AI 翻译 > 机器翻译，低优先级不覆盖高优先级，所有翻译结果均保留
- **自动流水线**：一键执行提取、翻译、打包全流程，分步输出详细统计
- **资源包生成**：自动生成 Minecraft 资源包，按模组原格式输出，可直接在游戏中启用
- **多语言界面**：命令输出跟随游戏语言，支持简体中文、繁体中文、英文
...
├── config.txt              # 配置文件
├── data/
│   ├── extracted.json      # 已提取的语言数据（多语言共用，语言代码为顶层键）
│   ├── translated.json     # 已翻译的语言数据（多语言共用）
│   └── packed.json         # 已打包的语言数据（多语言共用）
├── AItranslation_*.txt     # AI 待翻译文件（按目标语言分文件）
├── AItranslationResult_*.txt  # AI 翻译结果汇总文件
├── archive/ai/             # 已处理的 AI 文件归档
├── failed_pastes/          # 解析失败片段存档
├── i18n-dic/               # 词典目录（可放置多个 JSON 词典文件，自动合并）
└── logs/                   # 日志目录
```
...
1. 进入游戏世界（此时 MMT 会自动提取需要翻译的内容）
2. 打开游戏目录，找到 `mmt/AItranslation_[语言代码]_[编号].txt`
3. 将该文件内容发送给 AI，并等待翻译结果
4. 复制 AI 翻译结果，可以选择：
   - 粘贴并覆盖原 `AItranslation_*.txt` 文件
   - 或粘贴到 `AItranslationResult_*.txt` 文件中
5. 执行 `/mmt auto` 自动识别翻译并打包
6. 在游戏中启用生成的资源包（位于 `resourcepacks/MMT_<语言代码>/`）

### 进阶操作
1. **词典翻译**：前往 `https://github.com/CFPATools/i18n-dict/releases/` 下载词典文件，将词典文件放置在 `mmt/i18n-dic/` 目录下。此时MMT会优先使用词典翻译。**注意：词典仅适用于目标语言为中文（zh_cn / zh_tw）且源语言为英文（en_us）的场景。**
